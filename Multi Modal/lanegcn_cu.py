...
class PredLoss(nn.Module):
    ...
    def forward(self, out: Dict[str, List[Tensor]], gt_preds: List[Tensor], has_preds: List[Tensor]) -> Dict[str, Union[Tensor, int]]:
        ...
        for i in range(len(reg)):
            ...
            closs = self.cls_loss(dist_i,zs_i)
...
class Loss(nn.Module):
    def __init__(self, config, co_un_l1, co_fun_l1, tun):
        ...
        self.config = config
        self.pred_loss = PredLoss(config, co_un_l1, co_fun_l1, tun)
...
def get_model():
    ...
    loss = Loss(config, co_un_l1, co_fun_l1, tun).cuda()
...
def tun(dist, zs):
    dist = dist.detach()
    closs_t = torch.abs((dist.sum(-1)) - (zs.sum(-1)))
    closs = torch.sum(closs_t)
    return closs
