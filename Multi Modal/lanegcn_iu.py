...
class PredNet(nn.Module):
    ...
    def forward(self, actors: Tensor, actor_idcs: List[Tensor], actor_ctrs: List[Tensor]) -> Dict[str, List[Tensor]]:
        ...
        for i in range(len(self.pred)):
            ...
            featsun = self.att_dest(actors, torch.cat(actor_ctrs, 0), dest_ctrs_i)
            uns.append(self.un[0](featsun))
        uncertaintyt = torch.cat([x.unsqueeze(1) for x in uns], 1)

        
        uncertainty = uncertaintyt.reshape(uncertaintyt.size(0), uncertaintyt.size(1), -1, 1)
        exp_uncertainty = torch.exp(uncertainty).sum(-1)
        cls = exp_uncertainty[:,:,-1]

        cls, sort_idcs = cls.sort(1, descending=False)
        row_idcs = torch.arange(len(sort_idcs)).long().to(sort_idcs.device)
        row_idcs = row_idcs.view(-1, 1).repeat(1, sort_idcs.size(1)).view(-1)
        sort_idcs = sort_idcs.view(-1)
        reg = reg[row_idcs, sort_idcs].view(cls.size(0), cls.size(1), -1, 2)
        uncertainty = uncertainty[row_idcs, sort_idcs].view(cls.size(0), cls.size(1), -1, 1)
...
class AttDest2(nn.Module):
    def __init__(self, n_agt: int):
        ...
        self.en1 = nn.Sequential(
            nn.Linear(2 * config["num_preds"], n_agt),
            nn.ReLU(inplace=True),
            Linear(n_agt, n_agt, norm=norm, ng=ng),
        )

        self.en2 = nn.Sequential(
            nn.Linear(n_agt, n_agt),
            nn.ReLU(inplace=True),
            Linear(n_agt, n_agt, norm=norm, ng=ng),
        )
    ...
    def forward(self, uncertainty: Tensor, actorf: Tensor) -> Tensor:
        fun = self.en1(uncertainty)
        fun = self.en2(fun)

        agts = torch.cat((fun, actorf), 1)
        agts = self.agt(agts)
        return agts


class PredLoss(nn.Module):
    def __init__(self, config, un_l1, fun_l1, tun):
        super(PredLoss, self).__init__()
        self.config = config
        self.reg_loss = lambda a,b,c:un_l1(a,b,c)
        self.freg_loss = lambda a,b,c:fun_l1(a,b,c)
        self.cls_loss = lambda a,b,c,d:tun(a,b,c,d)
    ...
    def forward(self, out: Dict[str, List[Tensor]], gt_preds: List[Tensor], has_preds: List[Tensor]) -> Dict[str, Union[Tensor, int]]:
        ...
        uncertainty = torch.cat([x for x in uncertainty], 0)
        ...
        closs = self.cls_loss(reg,gt_preds,uncertainty,has_preds)
        
        loss_out["cls_loss"] += coef * closs
        loss_out["num_cls"] += has_preds.sum().item()

        reg = reg[row_idcs, min_idcs]
        uncertainty = uncertainty[row_idcs, min_idcs]
        coef = self.config["reg_coef"]
        f_weight = self.config["f_weight"]
        loss_out["reg_loss"] += coef * self.reg_loss(
            reg[has_preds], gt_preds[has_preds], uncertainty[has_preds]
        )
        regf = reg
        regf[:,0:29,:] = 0
        gt_predsf = gt_preds
        ...
        uncertaintyf[:,0:29,:] = 0
        loss_out["reg_loss"] += f_weight * self.freg_loss(
            regf[has_preds], gt_predsf[has_preds], uncertaintyf[has_preds]
        )
...
class Loss(nn.Module):
    def __init__(self, config, un_l1, fun_l1, tun):
        ...
        self.config = config
        self.pred_loss = PredLoss(config, un_l1, fun_l1, tun)
...
def get_model():
    ...
    loss = Loss(config, un_l1, fun_l1, tun).cuda()
...
def un_l1(final_traj = 1, target_traj = 1, uncertainty = 1):
    dist = torch.abs(final_traj-target_traj)
    dist = dist * torch.exp(-uncertainty) + uncertainty #l1 uncertainty
    loss = torch.sum(dist)
    return loss

def fun_l1(final_traj = 1, target_traj = 1, uncertainty = 1):
    dist = torch.abs(final_traj-target_traj)
    dist = dist * torch.exp(-uncertainty) + uncertainty #l1 uncertainty
    loss = torch.sum(dist)
    return loss
...
def tun(final_traj, target_traj, uncertainty, has_preds):
    closs = 0
    n_mod = uncertainty.size(1)
    p_target_traj = target_traj[has_preds]
    for i in range(n_mod):
        u_uncertainty = uncertainty[:,i,:,:]
        u_uncertainty = u_uncertainty[has_preds]
        u_final_traj = final_traj[:,i,:,:]
        u_final_traj = u_final_traj[has_preds]
        dist = torch.abs(u_final_traj-p_target_traj).sum(-1)
        closs_t = torch.abs(dist - (torch.exp(u_uncertainty).sum(-1)))
        closs = closs + torch.sum(closs_t)
    closs = closs / n_mod
    return closs
