...
config["lr"] = [1e-3, 1e-4, 1e-5, 5e-6, 1e-6, 5e-7, 1e-7]
config["lr_epochs"] = [22,28,32,36,40,50]
...
config["test_split"] = os.path.join(data_path, "dataset/test_obs/data")
...
class PredNet(nn.Module):
    ...
    def forward(self, actors: Tensor, actor_idcs: List[Tensor], actor_ctrs: List[Tensor]) -> Dict[str, List[Tensor]]:
        ...
        for i in range(len(self.un)):
            uns.append(self.un[i](feats))
        uncertainty = torch.cat([x.unsqueeze(1) for x in uns], 1)
        uncertainty = uncertainty.view(uncertainty.size(0), uncertainty.size(1), -1, 2) #[N,K,T,2]
        ...
        for i in range(len(actor_idcs)):
            ...
            temp_un_max, _ = torch.max(temp_un, dim=0,keepdim=True) #for linear
            temp_un = temp_un - temp_un_max
...
class PredLoss(nn.Module):
    def __init__(self, config, un_l1):
...
class Loss(nn.Module):
    def __init__(self, config, un_l1):
        ...
        self.config = config
        self.pred_loss = PredLoss(config, un_l1)
...
class PostProcess(nn.Module):
    ...
    def display(self, metrics, dt, epoch, lr=None):
        ...
        print(
            "loss %2.4f reg %2.4f, ade1 %2.4f, fde1 %2.4f, ade %2.4f, fde %2.4f"
            % (loss, reg, ade1, fde1, ade, fde)
        )
        print()
...
def un_l1(final_traj = 1, target_traj = 1, uncertainty = 1):
    ...
    dist = dist * torch.exp(-uncertainty) + uncertainty 
    dist = dist.mean(-1).mean(-1).mean(-1)
